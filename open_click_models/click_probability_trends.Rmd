...
The Gini Index ranges from 0 to 1, with 1 being perfect inequality. In this case, the distribution of clicks among the subscribers seems unequal; according to the curve, the top 10% of people account for 75% of clicks.
...
# Trends in the Click Probability

## Overview of the Data Used

We focused on the probability that a given subscriber will click on at least one link in a given newsletter. In addition to the variables used in the open probability model, we also look at the following factors affecting the click probability:

* Number of words in newsletter.
* Number of links in newsletter.
...
Below are 10 sample observations from the dataset, containing the variables mentioned above. The last variable, clicks, is the response variable of interest. 1 indicates that the subscriber clicked on any link in a given newsletter; 0 indicates that the subscriber didn't click on any link in a given newsletter.

<!-- double-check how clicks were calculated -->
...
hour_tab <- table(subscriber_open$week_open, hour_grp)
```
...
The below barplot shows the proportion of subscribers that opened the newsletter, given that the newsletter was sent to them within a specific time interval. The barplot suggests that there are two time intervals with higher open proportions.
...
ggplot(hour_dat, aes(x = hour_grp, y = hour_prop)) + 
  ...
  ggtitle("Proportion of Opens by Time Interval") + 
  xlab("Time Interval") + 
  ylab("Open Proportion") + 
...
The following plot shows the relative effect of the time of day the newsletter is sent out on the open probability (a negative relative effect corresponds to a decrease in probability, and a positive relative effect corresponds to an increase in probability) during the pandemic. It appears that the optimal times are about 10:30 in the morning and 17:10 in the evening. 
...
Note: I do not show graphs for the effect of hour of day before the pandemic, because the estimates are not very precise. 
...
plot_smooth(m3, view = "days_since_start", cond = list(covid = "Before"),
            transform = expit, xlim = c(0, 436),
            main = "Open Probability over Time Before COVID",
            ylab = "Probability", xlab = "Date")
```

```{r}
plot_smooth(m3, view = "days_since_start", cond = list(covid = "After"),
            transform = expit, xlim = c(436, 728),
            main = "Open Probability over Time After COVID",
            ylab = "Probability", xlab = "Date")
```
...
```{r}
plot_smooth(m3, view = "mins_since_midnight", cond = list(covid = "Before"),
            transform = expit,
            ylab = "Probability", xlab = "Date")
```

```{r}
plot_smooth(m3, view = "mins_since_midnight", cond = list(covid = "After"),
            transform = expit,
            ylab = "Probability", xlab = "Date")
```

```{r}
plot_smooth(m3, view = "subject_length",
            transform = expit,
            ylab = "Probability")
```

```{r}
plot_smooth(m3, view = "num_words",
            transform = expit,
            ylab = "Probability")
```

```{r}
plot_smooth(m3, view = "num_links",
            transform = expit,
            ylab = "Probability")
```

```{r}
plot_smooth(m3, view = "num_clickable_pics",
            transform = expit,
            ylab = "Probability")
```

```{r}
plot_smooth(m3, view = "num_unclickable_pics",
            transform = expit,
            ylab = "Probability")
